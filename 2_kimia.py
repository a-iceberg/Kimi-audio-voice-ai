...
class KimiAudio(object):
    ...
    def _generate_loop(
        ...
        decoder_position_ids = (
            torch.arange(
                0, decoder_input_audio_ids.shape[1], device="cpu"
            )
            .unsqueeze(0)
            .long()
        )
    ...
    def generate(
        ...
        audio_input_ids = audio_input_ids.to("cpu")
        text_input_ids = text_input_ids.to("cpu")
        is_continuous_mask = is_continuous_mask.to("cpu")
        audio_features = [f.to("cpu") for f in audio_features]
    ...
    def detokenize_audio(self, audio_tokens):
        ...
        audio_tokens = audio_tokens.to("cpu")
